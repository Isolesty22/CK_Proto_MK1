...
public class CustomPool<T> where T : MonoBehaviour
{
    ...
    /// <summary>
    /// 오브젝트를 해당 위치에 스폰합니다. 회전값은 Vector3.zero이고, 부모는 null입니다.
    /// </summary>
    /// <param name="_position">스폰할 오브젝트의 위치.</param>
    public T SpawnThis(Vector3 _position)
    {
        T tempObject = default(T);
        Transform tempTransform = null;
        lock (objectQueue)
        {
            ...
            //위치, 회전값 설정
            tempTransform.SetPositionAndRotation(_position, Quaternion.Euler(Vector3.zero));
            //tempTransform.rotation = Quaternion.Euler(_rotation);
